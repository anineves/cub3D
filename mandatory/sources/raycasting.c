...
/*   Updated: 2023/09/16 18:50:01 by anaraujo         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
ray->side: Indica se o raio atingiu uma parede vertical (0) ou horizontal (1).
...
ray->wall_x -= floor(ray->wall_x): Esta linha ajusta ray->wall_x subtraindo 
a parte inteira da coordenada da parede. Isso é usado para lidar com texturas de parede, 
...
map->tex_x = (int)(ray->wall_x * MAPS): Calcula a coordenada x na textura que deve
ser mapeada na parede, multiplicando a coordenada na parede (ray->wall_x) 
por MAPS e convertendo o resultado em um inteiro.

(ray->side == 0 && ray->dir_x < 0) || (ray->side == 1 && ray->dir_y > 0): verifica se
o raio atingiu uma parede norte ou este e, se for o caso, ajusta map->tex_x 
para garantir que a textura seja mapeada corretamente.

map->step = 1.0 * MAPS / ray->line_height: Calcula o tamanho do passo para mapear
a textura na parede, com base na altura da linha da parede (ray->line_height) 
e na largura da textura (MAPS).

map->pos = (ray->draw_start - WINDOW_HEIGHT / 2 + ray->line_height / 2) * map->step: 
Calcula a posição inicial na textura para mapear na parede com base na altura da 
linha da parede e na posição de início de desenho da linha (ray->draw_start).
...
map->pos += map->step: Incrementa a posição na textura para a próxima linha da parede.

color = data->textures[map->tex_index][MAPS * map->tex_y + map->tex_x]: 
Obtém a cor da textura na coordenada (tex_x, tex_y) da textura seleciona e armazena em color.
